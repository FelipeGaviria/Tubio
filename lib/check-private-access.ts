...
export async function checkPrivateAccess(request: Request, gate: string, code: string) {
  ...
  const salt = process.env.IP_HASH_SALT ?? process.env.ORDER_COOKIE_SECRET ?? process.env.SOCIAL_COOKIE_SECRET;
  if (!salt) return { response: new Response(null, { status: 503 }), result: { error: "Configuración de seguridad incompleta" } };
