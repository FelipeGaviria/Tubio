...
export const metadata: Metadata = {
  title: { absolute: "Portafolio | Felipe Gaviria" },
  description: "Portafolio de Felipe Gaviria: estrategia de marca, identidad visual, ilustración, contenido y producción audiovisual.",
  authors: [{ name: "Felipe Gaviria" }],
  creator: "Felipe Gaviria",
  openGraph: {
    title: "Portafolio | Felipe Gaviria",
    description: "Estrategia de marca, identidad visual, ilustración, contenido y producción audiovisual.",
    url: "/portafolio",
    siteName: "Felipe Gaviria",
    locale: "es_CO",
    type: "website",
  },
  twitter: {
...
export default function PortfolioPage() {
  return (
    <main id="inicio" className="raw-portfolio">
      ...
      <section className="raw-portfolio-hero"><div className="raw-hero-top"><div><p className="raw-date">25 Jul, 2026</p><p className="raw-person">Felipe Gaviria Vásquez</p></div><div className="raw-hero-actions"><p className="raw-contact-data">Medellín, Colombia</p><PortfolioThemeToggle /><PortfolioShareButton /></div></div><div className="raw-hero-title"><div className="raw-role-title"><span>Estrategia de marca&nbsp;&nbsp; / &nbsp;&nbsp;Dirección creativa&nbsp;&nbsp; / &nbsp;&nbsp;Producción audiovisual</span><a href="mailto:felipegaviria17@hotmail.com">felipegaviria17@hotmail.com</a></div><h1>Portafolio</h1></div></section>
      ...
      {projects.map((project, index) => <section id={`proyecto-${index + 1}`} className={`raw-project-section raw-project-section-${index + 2}`} key={project}><p className="raw-label">0{index + 5} / Proyecto</p><h2>{project}</h2><p>{project === "Contenido para marcas" ? "Diseño de piezas, campañas y sistemas visuales que conectan la estrategia de cada marca con una presencia reconocible." : "Esta sección queda preparada para desarrollar el proyecto, sus imágenes, proceso, resultados y detalles."}</p>{project === "Contenido para marcas" && <><p className="raw-carousel-label">Carruseles</p><BrandCarousel /></>}<a href="#inicio">Volver arriba <span>?</span></a></section>)}
      <ManualCases />
      <IllustrationGallery />
