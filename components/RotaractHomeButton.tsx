...
export function RotaractHomeButton({ active, unlocked, onNavigate, onEdit }: { active: boolean; unlocked: boolean; onNavigate: () => void; onEdit: () => void }) {
  ...
  return <button type="button" className={`rotaract-home-button ${holding ? "is-holding" : ""} ${completed ? "hold-complete" : ""} ${unlocked ? "is-editing" : ""}`} aria-current={active ? "page" : undefined} aria-label={`Inicio. Mantén pulsado 4 segundos para ${unlocked ? "cerrar" : "desbloquear"} edición`} title={`Mantén 4 segundos para ${unlocked ? "cerrar" : "desbloquear"} edición`}
