...
function AppNavIcon({ name }: { name: AppSection }) { const paths = { home: <><path d="M4 11.2 12 4l8 7.2"/><path d="M6.5 10v9.5h11V10M10 19.5v-5h4v5"/></>, attendance: <><path d="M8 3.5h8M9 2h6v4H9zM6 4.5H5a2 2 0 0 0-2 2V20h18V6.5a2 2 0 0 0-2-2h-1"/><path d="m7 13 3 3 7-7"/></>, fillers: <><path d="M5 18.5 3.5 21v-5A8 8 0 1 1 7 19"/><path d="M8 11h.01M12 11h.01M16 11h.01"/></>, calendar: <><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M8 3v4M16 3v4M3 10h18M7 14h3M14 14h3M7 17h3"/></>, words: <><path d="M4 19 9.5 5h2L17 19M6 14h9"/><path d="M18 9h3M19.5 7.5v3"/></> }; return <svg aria-hidden="true" viewBox="0 0 24 24">{paths[name]}</svg>; }
...
export function AttendanceApp() {
  ...
  async function shareApp() {
    const shareData = { title: "Tu sesión online", text: "Asistencia, agenda, muletillas y palabras de la sesión de Toastmasters.", url: window.location.href };
    try {
  ...
  return <main data-club="toastmasters" data-section={activeSection} className={`attendance-page toastmasters-hub ${unlocked ? "is-editing" : "is-viewing"}`}>
    ...
    <div className="attendance-layout">
      <aside className="attendance-sidebar toastmasters-app-sidebar">
        ...
        <nav className="toastmasters-app-nav" aria-label="Secciones de Toastmasters">{([ ["attendance", "Asistencias"], ["fillers", "Muletillas"], ["home", "Inicio"], ["calendar", "Calendario"], ["words", "Palabras"] ] as const).map(([section, label]) => <button type="button" className={activeSection === section ? "active" : ""} key={section} onClick={() => setActiveSection(section)}><span><AppNavIcon name={section}/></span>{label}</button>)}</nav>
