...
type Member = { id: string; name: string; joined: string; retired?: string; firstSessionAt?: string };
...
export function AttendanceApp() {
  ...
  const members = (() => {
    const visible = data?.members.filter((m) => m.joined <= date && (!m.retired || m.retired > date)) ?? [];
  ...
  function add(e: FormEvent) { e.preventDefault(); if (future) return; const clean = name.trim(); if (!clean) return; setData((d) => d && ({ ...d, members: [...d.members, { id: `${Date.now()}-${Math.random()}`, name: clean, joined: date, firstSessionAt: firstSession ? date : undefined }] })); setName(""); setFirstSession(false); }
  ...
  return <main className="attendance-page">
    ...
    <div className="attendance-layout">
      ...
      <section className="attendance-workspace">
        <div className="attendance-heading"><div><p>Control semanal</p><h1>Asistencia a Sesiones</h1><span>Registra quién vino y detecta a tiempo a quien necesita acompañamiento.</span></div><form className="add-person" onSubmit={add}><input disabled={future} value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre del asistente" aria-label="Nombre del nuevo asistente" /><label className={`first-session-add ${firstSession ? "checked" : ""}`}><input type="checkbox" checked={firstSession} disabled={future} onChange={(event) => setFirstSession(event.target.checked)} /><span>✓</span> Primera sesión</label><button disabled={future}><Icon name="plus" /> Agregar persona</button></form></div>
