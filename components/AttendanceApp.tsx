  ...
  return <main data-club="toastmasters" data-section={activeSection} className={`attendance-page toastmasters-hub ${unlocked ? "is-editing" : "is-viewing"}`}>
    ...
    <div className="attendance-layout">
      ...
      <section className="attendance-workspace">
        <section className="toastmasters-home" hidden={activeSection !== "home"}><div className="toastmasters-welcome"><h1>Bienvenid@, Toastmaster</h1><span>Prepara la próxima sesión, acompaña a los socios y mantén vivo el aprendizaje en cada encuentro.</span></div><div className="toastmasters-dashboard-grid"><button type="button" onClick={() => setActiveSection("attendance")}><i><AppNavIcon name="attendance"/></i><div><small>Próxima sesión</small><strong>{pretty(shift(toIso(monday()), 1), true)}</strong><span>Gestionar asistencia</span></div></button><button type="button" onClick={() => setActiveSection("fillers")}><i><AppNavIcon name="fillers"/></i><div><small>Muletillas registradas</small><strong>{data.fillerEntries.reduce((sum, entry) => sum + entry.count, 0)}</strong><span>Practicar claridad al hablar</span></div></button><button type="button" onClick={() => setActiveSection("words")}><i><AppNavIcon name="words"/></i><div><small>Palabra de la sesión</small><strong>{data.weeklyWords[0]?.word ?? "Sin palabra"}</strong><span>{data.weeklyWords[0]?.meaning || "Elegir palabra para la sesión"}</span></div></button></div><div className="toastmasters-next-events"><div><p>Agenda</p><h2>Próximas fechas</h2></div><div className="rotaract-events">{upcoming.slice(0, 3).map((event) => <article className={proximityClass(event.date)} key={event.id}><time><strong>{new Date(`${event.date}T12:00:00`).getDate()}</strong><span>{new Intl.DateTimeFormat("es-CO", { month: "short" }).format(new Date(`${event.date}T12:00:00`))}</span></time><div><h3>{event.title}</h3><p>{[event.time, event.place].filter(Boolean).join(" · ") || "Detalles por confirmar"}</p></div></article>)}{!upcoming.length && <div className="rotaract-empty-event">Aún no hay próximas fechas.</div>}</div></div></section>
