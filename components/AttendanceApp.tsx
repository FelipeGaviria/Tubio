  ...
  return <main data-club="toastmasters" data-section={activeSection} className={`attendance-page toastmasters-hub ${unlocked ? "is-editing" : "is-viewing"}`}>
    ...
    <div className="attendance-layout">
      <aside className="attendance-sidebar toastmasters-app-sidebar">
        ...
        <nav className="toastmasters-app-nav" aria-label="Secciones de Toastmasters">{([ ["home", "⌂", "Inicio"], ["attendance", "✓", "Asistencias"], ["fillers", "…", "Muletillas"], ["calendar", "□", "Calendario"], ["words", "Aa", "Palabras"] ] as const).map(([section, symbol, label]) => <button type="button" className={activeSection === section ? "active" : ""} key={section} onClick={() => setActiveSection(section)}><span>{symbol}</span>{label}</button>)}</nav>
