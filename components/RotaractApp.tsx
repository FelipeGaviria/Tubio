...
import { RotaractTreasury } from "@/components/RotaractTreasury";
...
export function RotaractApp() {
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener("keydown", trapFocus); document.querySelector<HTMLButtonElement>(".rotaract-edit-lock")?.focus(); };
  ...
  async function leaveEditMode() { localStorage.removeItem(ACCESS_UNTIL_KEY); setUnlocked(false); setEditUnlockOpen(false); setPassword(""); setAccessError(false); setCalendarEditorOpen(false); setEditingEventId(null); setEventDraft({ title: "", date: "", time: "", place: "", note: "" }); await fetch("/api/access-check?gate=rotaract", { method: "DELETE" }).catch(() => undefined); }
  ...
  return <main data-club="rotaract" className="attendance-page rotaract-page">
    <header className="attendance-topbar"><button type="button" className="attendance-logo rotaract-header-logo club-logo-refresh" onClick={() => window.location.reload()} aria-label="Actualizar Rotaract"><Image src="/images/clubs/rotaract-nuevo-medellin.png" alt="Rotaract Nuevo Medellín" width={182} height={63} priority/></button><div className="attendance-top-actions"><button type="button" className={`club-icon-button rotaract-edit-lock ${unlocked ? "is-unlocked" : ""}`} aria-label={unlocked ? "Cerrar edición" : "Desbloquear edición"} aria-pressed={unlocked} onClick={() => { if (unlocked) void leaveEditMode(); else { setPassword(""); setAccessError(false); setEditUnlockOpen(true); } }}><Icon name={unlocked ? "unlock" : "lock"}/></button><ClubAppControls club="rotaract" appName="Rotaract Nuevo Medellín" /><button className="share-attendance rotaract-share" type="button" onClick={() => void shareClub()} aria-label="Compartir Rotaract Nuevo Medellín"><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.6 10.5 6.8-4M8.6 13.5l6.8 4"/></svg><span>{shared ? "¡Copiado!" : "Compartir"}</span></button><div className={`autosave ${syncStatus}`}><span/> {syncStatus === "connecting" ? "Conectando…" : syncStatus === "saving" ? "Guardando…" : syncStatus === "error" ? "Sin conexión" : ""}</div></div></header>
    <RotaractNavigation section={section} onSelect={selectSection} />
