  ...
  return <main className="attendance-page rotaract-page">
    ...
    <div className="attendance-layout">
      ...
      <section className="attendance-workspace">
        ...
        <section className={`rotaract-calendar ${calendarEditorOpen ? "is-editing" : ""}`}>
          <div className="rotaract-section-title">
            ...
            <div className="calendar-title-actions">
              ...
              <button className="calendar-lock-toggle" type="button" aria-expanded={calendarEditorOpen} onClick={() => { setCalendarEditorOpen(!calendarEditorOpen); if (calendarEditorOpen) { setEditingEventId(null); setEventDraft({ title: "", date: "", time: "", place: "", note: "" }); } }}><Icon name={calendarEditorOpen ? "unlock" : "lock"}/>{calendarEditorOpen ? "Cerrar edición" : "Editar calendario"}</button>
            </div>
          </div>
          ...
          {monthCalendarOpen && <div className="club-month-calendar">
            ...
            <div className="month-days">{monthDays.map((day) => { const dayEvents = data.events.filter((item) => item.date === day); const outside = day.slice(0, 7) !== calendarMonth; return <button type="button" key={day} className={`${outside ? "outside" : ""} ${day === today() ? "today" : ""} ${day === selectedCalendarDate ? "selected" : ""}`} onClick={() => { setSelectedCalendarDate(day); if (outside) setCalendarMonth(day.slice(0, 7)); }}><time dateTime={day}>{Number(day.slice(-2))}</time><span className="month-event-titles">{dayEvents.slice(0, 2).map((item) => <i className={proximityClass(item.date)} key={item.id}>{item.time && <b>{item.time}</b>} {item.title}</i>)}</span>{dayEvents.length > 2 && <small>+{dayEvents.length - 2} más</small>}<span className="month-event-dots" aria-hidden="true">{dayEvents.slice(0, 3).map((item) => <i className={proximityClass(item.date)} key={item.id}/>)}</span></button>; })}</div>
            <div className="selected-day-agenda"><div><small>Fecha seleccionada</small><strong>{pretty(selectedCalendarDate)}</strong></div>{selectedDayEvents.length ? <div className="selected-day-events">{selectedDayEvents.map((event) => <article key={event.id}><span/><div><strong>{event.title}</strong><small>{[event.time, event.place].filter(Boolean).join(" · ") || "Detalles por confirmar"}</small>{event.note && <p>{event.note}</p>}</div>{calendarEditorOpen && <button type="button" onClick={() => editEvent(event)} aria-label={`Editar ${event.title}`}><Icon name="edit"/></button>}</article>)}</div> : <p>No hay actividades programadas para este día.</p>}{calendarEditorOpen && <button className="add-on-selected-day" type="button" onClick={() => { setEditingEventId(null); setEventDraft({ title: "", date: selectedCalendarDate, time: "", place: "", note: "" }); }}><Icon name="plus"/> Agregar en esta fecha</button>}</div>
          </div>}
