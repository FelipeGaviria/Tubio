  ...
  return <main data-club="rotaract" className="attendance-page rotaract-page">
    ...
    <div ref={layoutRef} className={`attendance-layout rotaract-section-layout ${section === "asistencias" ? "with-history" : ""}`} data-section={section}>
      ...
      <section className="attendance-workspace" aria-label={rotaractSections.find((item) => item.id === section)?.label}>
        ...
        <div className="meeting-document-actions"><MeetingMinutes key={date} value={meeting.minutes} editable={unlocked} onChange={(minutes) => { minutesDrafts.current = { ...minutesDrafts.current, [date]: minutes }; localStorage.setItem(MINUTES_DRAFTS_KEY, JSON.stringify(minutesDrafts.current)); editMeeting({ minutes }); }} /><AttendanceReportButton report={{ minutes: meeting.minutes, club: "rotaract", title: "Rotaract Nuevo Medellín", date, held: meeting.held, meetingLabel: meeting.held ? "Reunión del club" : "No hubo reunión", syncPending: syncStatus !== "saved", people: members.map((member) => ({ name: member.name, role: member.applicant ? "Aspirante" : "Socio", mark: meeting.marks[member.id] })) }} /></div>
