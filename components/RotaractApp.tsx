...
export function RotaractApp() {
  ...
  return <main data-club="rotaract" className="attendance-page rotaract-page">
    <header className="attendance-topbar"><div className="attendance-logo rotaract-header-logo"><Image src="/images/clubs/rotaract-nuevo-medellin.png" alt="Rotaract Nuevo Medellín" width={182} height={63} priority/></div><div className="attendance-top-actions"><ClubAppControls club="rotaract" appName="Rotaract Nuevo Medellín" /><button className="share-attendance rotaract-share" type="button" onClick={() => void shareClub()} aria-label="Compartir Rotaract Nuevo Medellín"><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.6 10.5 6.8-4M8.6 13.5l6.8 4"/></svg><span>{shared ? "¡Copiado!" : "Compartir"}</span></button><div className={`autosave ${syncStatus}`}><span/> {syncStatus === "connecting" ? "Conectando…" : syncStatus === "saving" ? "Guardando…" : syncStatus === "error" ? "Sin conexión" : "Sincronizado"}</div></div></header>
    ...
    <footer className={`rotaract-edit-mode ${unlocked ? "is-active" : ""}`}>
      ...
      {unlocked ? <button type="button" onClick={() => void leaveEditMode()}>Salir del modo edición</button> : <button type="button" onClick={() => { setPassword(""); setAccessError(false); setEditUnlockOpen((current) => !current); }}>Modo edición</button>}
