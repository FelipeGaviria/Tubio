  ...
  return <main data-club="rotaract" className="attendance-page rotaract-page">
    ...
    <div ref={layoutRef} className={`attendance-layout rotaract-section-layout ${section === "asistencias" ? "with-history" : ""}`} data-section={section}>
      ...
      <section className="attendance-workspace" aria-label={rotaractSections.find((item) => item.id === section)?.label}>
        {unlocked && <div className={`rotaract-save-bar ${otherEditor || saveConflict ? "has-warning" : ""}`}><div>{saveConflict ? <><strong>Hay una versión más reciente</strong><small>Otro dispositivo guardó primero. Recarga la página para tomar esos cambios antes de continuar.</small></> : otherEditor ? <><strong>Ya hay alguien editando</strong><small>Guarda tus cambios para evitar sobreescribir información.</small></> : <><strong>Modo edición</strong><small>{dirty ? "Tienes cambios sin guardar." : "Todo está guardado."}</small></>}</div><button type="button" disabled={!dirty || syncStatus === "saving" || saveConflict} onClick={() => void saveChanges()}>{syncStatus === "saving" ? "Guardando…" : "Guardar cambios"}</button></div>}
        ...
        <div className="meeting-document-actions"><MeetingMinutes key={date} value={meeting.minutes} editable={unlocked} onRequestEdit={() => { setPassword(""); setAccessError(false); setEditUnlockOpen(true); }} onChange={(minutes) => { minutesDrafts.current = { ...minutesDrafts.current, [date]: minutes }; localStorage.setItem(MINUTES_DRAFTS_KEY, JSON.stringify(minutesDrafts.current)); editMeeting({ minutes }); }} /><AttendanceReportButton report={{ minutes: meeting.minutes, club: "rotaract", title: "Rotaract Nuevo Medellín", date, held: meeting.held, meetingLabel: meeting.held ? "Reunión del club" : "No hubo reunión", syncPending: dirty, people: members.map((member) => ({ name: member.name, role: member.applicant ? "Aspirante" : "Socio", mark: meeting.marks[member.id] })) }} /></div>
        ...
        {section === "inicio" && <div className="rotaract-home"><InteractiveRotaryWheel /></div>}
