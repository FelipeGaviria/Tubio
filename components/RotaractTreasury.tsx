...
export function RotaractTreasury({ members, unlocked, value, onChange, onRequestEdit }: { members: TreasuryMember[]; unlocked: boolean; value: TreasuryData; onChange: (value: TreasuryData) => void; onRequestEdit: () => void }) {
  ...
  return <section className="rotaract-treasury">
    ...
    <div className="treasury-period"><button type="button" onClick={() => moveMonth(-1)} aria-label="Mes anterior">‹</button><div><small>Año Rotaract {rotaryYear(month)}</small><strong>{monthLabel}</strong></div><button type="button" onClick={() => moveMonth(1)} aria-label="Mes siguiente">›</button></div>
    ...
    <p className="treasury-note">El año Rotaract va de agosto a julio. Los aspirantes no pagan cuota. Pago anticipado: primer semestre hasta el 30 de septiembre; segundo semestre hasta el 28 de febrero.</p>
    <div className="treasury-members"><h2>Socios del club</h2>{socios.map((member) => { const payment = value.payments.find((item) => item.memberId === member.id && covers(item, month)); const plan = value.plans[member.id] ?? "monthly"; const debt = debtFor(member); return <article key={member.id}><span className="treasury-avatar">{member.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span><div className="treasury-person"><strong>{member.name}</strong><small>{payment ? `Pagó ${money.format(payment.amount)} · ${payment.paidAt}` : "Pago pendiente"}</small><span className="treasury-debt"><button type="button" onClick={() => setVisibleDebts((current) => ({ ...current, [member.id]: !current[member.id] }))} aria-label={visibleDebts[member.id] ? `Ocultar deuda de ${member.name}` : `Ver deuda de ${member.name}`}>{visibleDebts[member.id] ? "◉" : "◎"}</button><small>Deuda acumulada: {visibleDebts[member.id] ? money.format(debt) : "••••••"}</small></span></div><div className="treasury-actions">{unlocked && <select aria-label={`Plan de ${member.name}`} value={plan} onChange={(event) => onChange({ ...value, plans: { ...value.plans, [member.id]: event.target.value as TreasuryPlan } })}><option value="monthly">Mensual</option><option value="semester">Semestral</option><option value="annual">Anual</option></select>}<button type="button" className={payment ? "treasury-unpay" : "treasury-pay"} onClick={() => togglePayment(member)}>{payment ? "Desmarcar" : "Marcar pago"}</button></div></article>; })}{!socios.length && <p>Aún no hay socios activos. Puedes agregarlos desde Reuniones.</p>}</div>
  </section>;
}
