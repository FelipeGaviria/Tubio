...
export function PrivateUnlock({ title, description, endpoint, destination }: { title: string; description: string; endpoint: string; destination: string }) {
  ...
  async function validate(nextCode: string) {
    if (requestPending.current || nextCode.length !== 4) return;
    ...
    try {
      ...
      if (!response.ok) { setError(result.locked ? "Demasiados intentos. Este acceso queda bloqueado durante 15 minutos." : `Código incorrecto. ${result.attemptsRemaining ?? 0} intentos disponibles.`); setCode(""); return; }
