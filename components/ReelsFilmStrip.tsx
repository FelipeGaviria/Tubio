...
export default function ReelsFilmStrip() {
  ...
  useEffect(() => { const timer = window.setInterval(() => { const element = track.current; if (!element) return; const frame = element.querySelector<HTMLElement>(".raw-film-frame"); const step = (frame?.offsetWidth ?? 260) + 18; if (element.scrollLeft + element.clientWidth >= element.scrollWidth - step) element.scrollTo({ left: 0, behavior: "smooth" }); else element.scrollBy({ left: step, behavior: "smooth" }); }, 4400); return () => window.clearInterval(timer); }, []);
  return <section className="raw-reels"><div className="raw-reels-heading"><p className="raw-label">Selección audiovisual</p><h3>Reels y videos.</h3><p>Desde producción, hasta edición y delivery.</p></div><div className="raw-film-viewport" ref={track}><div className="raw-film-track">{videos.map((video, index) => <article className="raw-film-frame" key={video.href}><div className="raw-film-perforations" aria-hidden="true"><i /><i /><i /><i /><i /><i /></div><div className="raw-film-image raw-film-preview">{video.embed ? <iframe src={video.preview} title={`Previsualización ${video.title}`} loading="lazy" /> : <img src={video.preview} alt={`Miniatura de ${video.title}`} loading="lazy" />}</div><div className="raw-film-caption"><small>{video.title} · {video.source}</small><a href={video.href} target="_blank" rel="noreferrer" aria-label={`Abrir ${video.title} en ${video.source}`}>▶</a></div><div className="raw-film-perforations" aria-hidden="true"><i /><i /><i /><i /><i /><i /></div></article>)}</div></div></section>;
}