...
const verticalPieces: BrandPiece[] = [
  { brand: "El Burro", src: "/images/portfolio/marcas/burro1.png", alt: "El Burro, pieza 1" }, { brand: "El Burro", src: "/images/portfolio/marcas/burro2.png", alt: "El Burro, pieza 2" }, { brand: "El Burro", src: "/images/portfolio/marcas/burro3.png", alt: "El Burro, pieza 3" }, { brand: "El Burro", src: "/images/portfolio/marcas/burro4.png", alt: "El Burro, pieza 4" },
...
const widePieces: BrandPiece[] = [{ brand: "El Burro", src: "/images/portfolio/marcas/burro-banner.png", alt: "El Burro, banner" }, { brand: "Rotary", src: "/images/portfolio/marcas/rotary2.png", alt: "Rotary, campaña" }, { brand: "San Juan", src: "/images/portfolio/marcas/san-juan.png", alt: "San Juan, banner" }];
...
export default function BrandCarousel() {
  ...
  return <div className="raw-brand-carousel"><div className="raw-brand-carousel-top"><div><p className="raw-label">Marcas con las que he trabajado</p><p>Diseños, campañas y sistemas visuales en conversación con contextos, públicos y objetivos distintos.</p></div><div><button type="button" onClick={() => move(-1)} aria-label="Ver trabajos anteriores">←</button><button type="button" onClick={() => move(1)} aria-label="Ver más trabajos">→</button></div></div><div className="raw-brand-track" ref={track}>{verticalPieces.map((piece) => <Piece key={piece.src} piece={piece} onOpen={setSelected} />)}</div><div className="raw-brand-wide"><p className="raw-label">Formatos panorámicos</p><div>{widePieces.map((piece) => <Piece key={piece.src} piece={piece} onOpen={setSelected} wide />)}</div></div>{selected && <button type="button" className="raw-illustration-modal" onClick={() => setSelected(null)} aria-label="Cerrar diseño ampliado"><Image src={selected.src} alt={selected.alt} fill sizes="100vw" priority /></button>}</div>;
}