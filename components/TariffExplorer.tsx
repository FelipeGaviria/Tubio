...
const initialServices: Service[] = [
  { id: "storyboard", title: "Storyboard", category: "Animación", unit: "Por cuadro", intro: "La complejidad depende del plano, los personajes y el nivel de definición del fondo.", levels: [
    ...
    { name: "Medium", description: "Planos con varios personajes o fondos definidos.", value: 11 },
    { name: "Easy", description: "Primeros planos o planos medios con 1–2 personajes y fondos sencillos.", value: 6 },
  ]},
  { id: "animatic", title: "Animatic", category: "Animación", unit: "Por segundo", fps: "3 frames aprox. por segundo", intro: "Tarifa única para la construcción temporal del animatic.", levels: [
    { name: "Tarifa", description: "Referencia: 1 segundo equivale a 3 frames aproximadamente.", perSecond: 7, perFrame: 3 },
  ]},
  ...
  { id: "rough", title: "Rough", category: "Animación", unit: "Por segundo / frame", fps: "24 FPS de referencia", intro: "Animación rough cotizada de acuerdo con los dibujos requeridos por la escena.", levels: [
    { name: "Hard", description: "Mucha acción, 2 o más personajes; más de 12 cuadros por segundo.", perSecond: 29, perFrame: 29 / 24 },
    { name: "Medium", description: "Acción media, más de 1 personaje; entre 8 y 12 cuadros por segundo.", perSecond: 24, perFrame: 24 / 24 },
    { name: "Easy", description: "Poca o media acción, personajes casi estáticos o pequeños; entre 6 y 8 cuadros por segundo.", perSecond: 19, perFrame: 19 / 24 },
  ]},
  { id: "clean", title: "Clean / Color", category: "Animación", unit: "Por segundo / frame", fps: "24 FPS de referencia", intro: "Limpieza y color: se cobran los dibujos únicos que realmente deben trabajarse.", levels: [
    { name: "Hard", description: "Mucha acción y 2 o más personajes; entre 8 y 12 cuadros por segundo.", perSecond: 19, perFrame: 19 / 24 },
    { name: "Medium", description: "2 personajes (uno con poca acción) o 1 personaje con poca acción; entre 8 y 12 cuadros.", perSecond: 16, perFrame: 16 / 24 },
    { name: "Easy", description: "Poca acción, personajes casi estáticos, distantes o pequeños; entre 6 y 8 cuadros.", perSecond: 13, perFrame: 13 / 24 },
  ]},
  { id: "color", title: "Solo color", category: "Animación", unit: "Por segundo / frame", fps: "24 FPS de referencia", intro: "Aplicación de color sobre dibujos listos; se cuentan únicamente dibujos únicos.", levels: [
    ...
    { name: "Easy", description: "Poca acción, personajes casi estáticos, distantes o pequeños; entre 6 y 8 cuadros.", perSecond: 6, perFrame: 6 / 24 },
  ]},
  { id: "backgrounds", title: "Backgrounds", category: "Arte", unit: "Por fondo", intro: "Fondos cotizados por complejidad técnica y cantidad de capas.", levels: [
    { name: "Hard", description: "Fondo complejo con varias capas para hacer parallax.", value: 78 },
    { name: "Medium", description: "Fondo con algunas capas para hacer parallax.", value: 58 },
    { name: "Easy", description: "Fondo plano o abstracto, de pocas capas y colores.", value: 39 },
  ]},
...
export function TariffExplorer() {
  ...
  const ratePerFrame = (service: Service, item: Level) => service.fps === "24 FPS de referencia" && item.perSecond !== undefined ? item.perSecond / 24 : item.perFrame;
  const convert = (value: number) => currency === "COP" ? Math.round(value * trm) : Math.round(value);
          ...
          <div className="mero-detail-body">
            ...
            <div className="mero-rate-list">
              {service.levels.map((item, itemIndex) => <article key={item.name} data-level={item.name.toLowerCase()}><div><small>Complejidad</small><h3>{item.name}</h3><p>{item.description}</p></div><dl>
                ...
                {item.perFrame !== undefined && <div><dt>Por frame · {currency}</dt><dd>{editing && service.fps !== "24 FPS de referencia" ? <input aria-label={`${service.title} ${item.name} por frame en USD`} type="number" min="0" step="1" value={Math.round(item.perFrame)} onChange={(event) => changeRate(services.indexOf(service), itemIndex, "perFrame", Math.round(Number(event.target.value)))} /> : money(ratePerFrame(service, item)!)}</dd></div>}
