...
export function Header() {
  ...
  return <header className={`site-header ${open ? "menu-open" : ""}`}>
    ...
    <nav aria-label="Navegación principal">{site.navigation.map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)}>{item.label}</Link>)}</nav>
    <div className="header-actions"><Link className="header-profile-link" href="/portafolio" aria-label="Abrir Portafolio"><PersonIcon /></Link><a className="header-cta" href={whatsappUrl()} target="_blank" rel="noopener noreferrer">Contactar</a></div>
  </header>;
}