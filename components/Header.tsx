...
export function Header() {
  return <header className="site-header"><Link className="brand" href="/" aria-label={`${site.name} inicio`}><span className="brand-mark">TB</span><span>{site.name}</span></Link><nav aria-label="Navegacion principal">{site.navigation.map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}</nav><div className="header-actions"><Link className="header-profile-link" href="/portafolio" aria-label="Abrir Portafolio"><PersonIcon /></Link><a className="header-cta" href={whatsappUrl()} target="_blank" rel="noopener noreferrer">Contactar</a></div></header>;
}